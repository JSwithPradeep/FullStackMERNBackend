...
exports.userpost = async(req,res)=>{
    ...
    if(!fname || !lname || !email || !mobile || !gender || !location || !status || !file ){
        res.status(401).json("All Inputs is require")
    }
    try{
...
exports.useredit = async(req, res) =>{
    ...
    const { id } = req.params;
    const { fname, lname, email, mobile, gender, location, status, user} = req.body;
    const file = req.file  ? req.file.filename : user
...
exports.userstatus = async(req,res)=>{
    ...
    try{
        const userStatusupdate = await users.findByIdAndUpdate({_id:id},{status:date},{new:true});
